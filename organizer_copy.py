...
class Organizer:
	...
	def organize(self):
		...
		# # only print diff
		# nums   = np.array(self.packet_num)
		# diffs  = np.diff(nums)
		# bad    = np.where(diffs != 1)[0]

		# if bad.size == 0:
		# 	print("All packets in perfect sequence 👍")
		# else:
		# 	for idx in bad:
		# 		print(f"Gap at position {idx}: "
		# 			f"{nums[idx]} → {nums[idx+1]} "
		# 			f"(Δ={diffs[idx]})")
		# 	print("Total packets received ", len(self.packet_num))
